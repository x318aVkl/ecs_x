...
use crate::{commands::{StoredCommand, SystemMessage}, ecs_table::EcsTable, system::{IntoSystem, StoredSystem}, plugin::Plugin};
...
impl Scheduler {
    ...
    pub fn run(mut self) -> Result<(), u32> {
        ...
        if let Err(msg) = self.static_change_state(SystemStartup) {
            match msg {
                SystemMessage::Exit(code) => {
                    if code == 0 {return Ok(())} else {return Err(code)}
                },
                _ => {}
            }
        }
        ...
        if self.systems.contains_key(&TypeId::of::<Update>()) {
            loop {
